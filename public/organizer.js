...
const post = async (url, payload = {}) => {
  const response = await fetch(url, {
    ...
    body: JSON.stringify({ ...payload, organizerKey }),
  });
...
const getState = () =>
  fetch("/api/state", { headers: { "X-Organizer-Key": organizerKey } }).then(
    (response) => {
      if (!response.ok) throw new Error("Organizer authorization failed.");
...
function login() {
  ...
  document.querySelector("#authorize").onclick = async () => {
    ...
    try {
      await post("/api/organizer/verify");
      sessionStorage.setItem("organizerKey", organizerKey);
      renderRoom();
    } catch (error) {
...
function monitorRows(sessions) {
  return sessions.length
    ? sessions
        .map(
          (item) =>
            `<tr><td>${escapeHtml(item.participantName)}</td><td><span class="tag ${item.connected ? "" : "warn"}">${item.connected ? "ONLINE" : "OFFLINE"}</span></td><td>${item.sessionId.slice(0, 8).toUpperCase()}</td><td><span class="tag ${item.tabStatus !== "ACTIVE" ? "warn" : ""}">${escapeHtml(item.tabStatus || item.state)}</span></td><td><button class="btn secondary remove-participant" data-session-id="${item.sessionId}">REMOVE</button></td></tr>`,
        )
        .join("")
...
function leaderboardRows(sessions) {
  ...
  return completed.length
    ? completed
        .map(
          (item, index) =>
            `<tr><td>${index + 1}</td><td>${escapeHtml(item.participantName)}</td><td><strong>${item.score} / 50</strong></td><td>${item.state === "submitted" ? "SUBMITTED" : escapeHtml(item.state)}</td></tr>`,
        )
        .join("")
    : '<tr><td colspan="4" class="muted">Scores appear here after participants submit.</td></tr>';
}
...
async function renderRoom() {
  try {
    ...
    const candidates = [
      ...new Set([
        ...roster.map((student) => student.name),
        ...state.sessions.map((item) => item.participantName),
      ]),
    ];
    ...
    document
      .querySelector(".hero")
      .insertAdjacentHTML("afterend", dashboardStats(state));
    ...
    document
      ...
      .forEach(
        (button) =>
          (button.onclick = () =>
            editQuestion(Number(button.dataset.questionId), questions)),
      );
    document
      ...
      .forEach(
        (button) =>
          (button.onclick = () =>
            deleteQuestion(Number(button.dataset.questionId))),
      );
...
function showRoomError(message) {
  ...
  app.insertAdjacentHTML(
    "afterbegin",
    `<div class="shell"><div id="room-error" class="notice danger">${escapeHtml(message)}</div></div>`,
  );
}

function selectedNames() {
  return [...document.querySelectorAll("input[data-name]:checked")].map(
...
async function deleteQuestion(questionId) {
  ...
  try {
    await post("/api/organizer/questions/delete", { questionId });
    await renderRoom();
  } catch (error) {
    showRoomError(error.message);
  }
...
async function refreshRoom() {
  try {
    ...
    document
      .querySelectorAll(".remove-participant")
      .forEach(
        (button) =>
          (button.onclick = () => removeParticipant(button.dataset.sessionId)),
      );
    ...
    if (selectedCount)
      selectedCount.textContent = (
        state.competition.selectedParticipantIds || []
      ).length;
    const onlineCount = document.querySelector("#stat-online");
    if (onlineCount)
      onlineCount.textContent = state.sessions.filter(
        (item) => item.connected,
      ).length;
    const submittedCount = document.querySelector("#stat-submitted");
    if (submittedCount)
      submittedCount.textContent = state.sessions.filter(
        (item) => item.state === "submitted",
      ).length;
    ...
    if (clock)
      clock.textContent = state.competition.endsAt
        ? `ends ${new Date(state.competition.endsAt).toLocaleTimeString()}`
        : "";
  } catch {}
}
function bindControls(roster) {
  ...
  document.querySelector("#start").onclick = async () => {
    try {
      ...
      if (!names.length)
        throw new Error("Select at least one participant before starting.");
      await post("/api/organizer/start", {
        selectedParticipantIds: names,
        durationMinutes: Number(
          document.querySelector("#duration-minutes").value,
        ),
      });
      await renderRoom();
    } catch (error) {
  ...
  document.querySelector("#results").onclick = async () => {
    try {
      await post("/api/organizer/reveal-results", {
        revealDurationMinutes: Number(
          document.querySelector("#reveal-duration").value,
        ),
      });
      await renderRoom();
    } catch (error) {
  ...
  document.querySelector("#answers").onclick = async () => {
    try {
      await post("/api/organizer/reveal-answers", {
        revealDurationMinutes: Number(
          document.querySelector("#reveal-duration").value,
        ),
      });
  ...
  document.querySelector("#select-all").onclick = () =>
    post("/api/organizer/roster", {
      students: roster.map((item) => item.name),
      selectedParticipantIds: roster.map((item) => item.name),
    }).then(renderRoom);
  document.querySelector("#add-student").onclick = () => {
    const name = document.querySelector("#student-name").value.trim();
    if (name)
      post("/api/organizer/roster", {
        students: [...roster.map((item) => item.name), name],
        selectedParticipantIds: [...roster.map((item) => item.name), name],
      }).then(renderRoom);
  };
  document.querySelector("#add-question").onclick = async () => {
    const options = ["a", "b", "c", "d"].map((letter) =>
      document.querySelector(`#option-${letter}`).value.trim(),
    );
    const question = {
      prompt: document.querySelector("#question-text").value.trim(),
      options,
      correct: Number(document.querySelector("#correct-index").value),
    ...
    try {
      const editingId =
        document.querySelector("#add-question").dataset.editingId;
      await post(
        editingId
          ? "/api/organizer/questions/update"
...
async function removeParticipant(sessionId) {
  try {
    await post("/api/organizer/session/delete", { sessionId });
    await renderRoom();
  } catch (error) {
    showRoomError(error.message);
  }
