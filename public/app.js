...
const channel =
  "BroadcastChannel" in window
    ? new BroadcastChannel("code-battle-quiz")
    : null;
...
async function join() {
  const name =
    document.querySelector("#participant-name").value.trim() || "Participant";
  try {
    const result = await post("/api/session/register", {
    ...
    if (error.code === "MULTIPLE_TAB") {
      showBlocker(
        "ANOTHER QUIZ TAB IS ALREADY ACTIVE",
        "Please return to your original quiz tab. This tab cannot become active automatically.",
      );
    } else if (error.code === "PARTICIPANT_ALREADY_LOGGED_IN") {
      showBlocker(
        "PARTICIPANT ALREADY LOGGED IN",
        "This participant name is already active in another quiz session. Use your own assigned participant name.",
      );
    } else if (error.code === "NOT_SELECTED") {
      showBlocker(
        "STUDENT NOT SELECTED",
        "This student is not currently selected for the active test. Please wait for the organizer to start the room or choose a valid student.",
      );
    } else {
  ...
  heartbeatTimer = setInterval(async () => {
    try {
      ...
      if (
        ...
        clearInterval(heartbeatTimer);
        alert(
          "The server has restarted or your session has expired. You need to rejoin.",
        );
...
function renderStopped() {
  clearInterval(examTimer);
  base(
    `<section class="hero"><div class="eyebrow">Test closed</div><h1>The test has ended.</h1><p class="subhead">The organizer stopped the test or the server timer reached zero. Your saved answers remain recorded on the server.</p></section>`,
  );
}
...
async function submitCurrentAttempt() {
  try {
    session = await post("/api/session/submit", {
      participantId: session.participantId,
      sessionId: session.sessionId,
      tabId,
    });
...
function showViolationWarning(message) {
  ...
  warningVisible = true;
  base(
    `<section class="blocker"><div class="blocker-card"><div class="eyebrow">Strict quiz warning ${violationCount}/${MAX_VIOLATIONS}</div><h1>Return to the quiz.</h1><p class="subhead" style="color:#d7e0d8">${escapeHtml(message)} This warning has been recorded. After ${MAX_VIOLATIONS} warnings, your attempt is submitted automatically.</p><div class="actions"><button class="btn" id="return-to-quiz">RETURN TO QUIZ</button></div></div></section>`,
  );
...
function renderFullscreenGate() {
  base(
    `<section class="hero"><div class="eyebrow">Fullscreen mode enabled</div><h1>Enter fullscreen to start.</h1><p class="subhead">Fullscreen is an additional deterrent, not a security mechanism. Your quiz session remains server-controlled.</p><button class="btn" id="fullscreen-start">ENTER FULLSCREEN</button></section>`,
  );
...
function renderQuestion() {
  ...
  const currentIndex = Math.min(
    Math.max(Number(session.currentQuestion) || 0, 0),
    list.length - 1,
  );
  const question =
    list[currentIndex] || list.find((item) => !session.answers[item.id]);
  ...
  document.querySelector("#previous-question").onclick = () => {
  ...
  document.querySelector("#submit-test").onclick = () => {
    if (
      confirm(
...
async function submitAnswer(question, payload) {
  try {
    session = await post("/api/session/answer", {
      participantId: session.participantId,
      sessionId: session.sessionId,
      tabId,
...
async function renderComplete() {
  ...
  session = state.session;
  const revealOpen =
    (state.competition.state === "results" ||
      state.competition.state === "answers") &&
    (!state.competition.revealEndsAt ||
      Date.now() < state.competition.revealEndsAt);
  ...
  if (revealOpen && state.competition.revealEndsAt)
    setTimeout(
      renderParticipant,
      Math.max(1000, state.competition.revealEndsAt - Date.now()),
    );
}
...
async function renderOrganizer() {
  ...
  document.querySelector("#add-question").onclick = () => {
    ...
    if (
      ...
      alert(
        "Please fill in the question prompt, all four options, and a valid correct index between 0 and 3.",
      );
      return;
    }
...
channel?.addEventListener("message", (event) => {
  if (
    event.data?.type === "ACTIVE_TAB" &&
    event.data.participantId === session?.participantId &&
    ...
    post("/api/session/event", {
      ...
      tabId,
      type: "MULTIPLE_TAB",
      detectedTabId: event.data.tabId,
    }).catch(() => {});
    showBlocker(
      "MULTIPLE QUIZ WINDOWS DETECTED",
      "CODE BATTLE 2026 requires the quiz to run in a single browser tab. Please close all other CODE BATTLE tabs/windows.",
    );
  }
});

document.addEventListener("visibilitychange", () => {
  if (localMode === "participant" && document.hidden)
    recordViolation("NAVIGATION_VIOLATION", "You left the quiz tab.");
});
document.addEventListener("fullscreenchange", () => {
  if (
    ...
    recordViolation("FULLSCREEN_VIOLATION", "Fullscreen mode was exited.");
});
...
window.addEventListener("popstate", () => {
  ...
  recordViolation(
    ...
    "You tried to go back from the quiz.",
  );
});
window.addEventListener("beforeunload", () =>
  channel?.postMessage({ type: "TAB_CLOSING", tabId }),
);
window.addEventListener("focus", () => {
  if (localMode === "participant") broadcast();
});
