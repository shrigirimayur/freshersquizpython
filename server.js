...
const defaultQuestions = [
  ...
  { id: 2, prompt: 'What does CSS stand for?', options: ['Cascading Style Sheets', 'Computer Style Syntax', 'Creative Sheet System', 'Colorful Style Sheets'], correct: 0, explanation: 'CSS is Cascading Style Sheets.' },
  { id: 3, prompt: 'Which data structure uses FIFO ordering?', options: ['Stack', 'Queue', 'Tree', 'Graph'], correct: 1, explanation: 'A queue removes the item that has waited longest.' },
  { id: 4, prompt: 'What is the default port for HTTPS?', options: ['80', '21', '443', '8080'], correct: 2, explanation: 'HTTPS conventionally uses TCP port 443.' },
  { id: 5, prompt: 'Which keyword declares a constant in JavaScript?', options: ['fixed', 'let', 'const', 'static'], correct: 2, explanation: 'const creates a binding that cannot be reassigned.' },
  { id: 6, prompt: 'Which protocol is used for secure browser connections?', options: ['FTP', 'HTTPS', 'SMTP', 'SSH'], correct: 1, explanation: 'HTTPS is HTTP layered over TLS.' },
  { id: 7, prompt: 'What does JSON stand for?', options: ['JavaScript Object Notation', 'Joined System Object Network', 'Java Source Open Nodes', 'JavaScript Oriented Names'], correct: 0, explanation: 'JSON means JavaScript Object Notation.' },
  { id: 8, prompt: 'Which Git command creates a new branch?', options: ['git fork', 'git branch', 'git split', 'git clone'], correct: 1, explanation: 'git branch creates or lists branches.' },
  { id: 9, prompt: 'Which layer handles IP routing?', options: ['Application', 'Transport', 'Network', 'Presentation'], correct: 2, explanation: 'The network layer handles logical addressing and routing.' },
  { id: 10, prompt: 'What does REST commonly describe?', options: ['A database engine', 'An API architectural style', 'A CSS framework', 'A testing language'], correct: 1, explanation: 'REST is an architectural style for networked applications.' },
  { id: 11, prompt: 'Which value is falsy in JavaScript?', options: ['[]', '{}', '"0"', '0'], correct: 3, explanation: 'The number 0 is falsy; arrays, objects, and non-empty strings are truthy.' },
  { id: 12, prompt: 'What is the purpose of a database index?', options: ['Encrypt rows', 'Speed up lookups', 'Delete duplicates', 'Create backups'], correct: 1, explanation: 'Indexes accelerate reads by organizing lookup paths.' },
  { id: 13, prompt: 'Which HTTP method is conventionally idempotent for replacement?', options: ['POST', 'PATCH', 'PUT', 'CONNECT'], correct: 2, explanation: 'PUT replaces a resource and repeating it has the same intended effect.' },
  { id: 14, prompt: 'What does DNS translate?', options: ['Ports to protocols', 'Names to IP addresses', 'HTML to CSS', 'Users to roles'], correct: 1, explanation: 'DNS resolves domain names to network addresses.' },
  { id: 15, prompt: 'Which is a symmetric encryption algorithm?', options: ['AES', 'RSA', 'DSA', 'ECDSA'], correct: 0, explanation: 'AES uses the same secret key to encrypt and decrypt.' },
  { id: 16, prompt: 'What does a 2xx HTTP response indicate?', options: ['Redirect', 'Client error', 'Success', 'Server error'], correct: 2, explanation: 'The 2xx class indicates successful processing.' },
  { id: 17, prompt: 'Which tool packages applications into isolated units?', options: ['Docker', 'Figma', 'Postman', 'Vite'], correct: 0, explanation: 'Docker packages applications into containers.' },
  { id: 18, prompt: 'What is a race condition?', options: ['A slow query', 'Timing-dependent shared-state behavior', 'A syntax error', 'A network protocol'], correct: 1, explanation: 'Race conditions occur when timing changes the outcome of competing operations.' },
  { id: 19, prompt: 'Which browser API lets same-origin tabs exchange messages?', options: ['BroadcastChannel', 'Canvas', 'FileReader', 'History'], correct: 0, explanation: 'BroadcastChannel broadcasts messages between browsing contexts on the same origin.' },
  { id: 20, prompt: 'What is the primary authority for quiz tab ownership here?', options: ['The browser title', 'The client UI', 'The server', 'The URL hash'], correct: 2, explanation: 'The server authorizes the active tab and rejects unauthorized submissions.' }
];
...
function parseBody(req) {
  return new Promise((resolve, reject) => {
    let raw = '';
    req.on('data', chunk => { raw += chunk; if (raw.length > 100_000) reject(new Error('Payload too large')); });
    req.on('end', () => { try { resolve(raw ? JSON.parse(raw) : {}); } catch { reject(new Error('Invalid JSON')); } });
    req.on('error', reject);
  });
}
function findSession(body) { return sessions.get(body.sessionId); }
...
async function route(req, res) {
  ...
  if (req.method === 'POST') {
    ...
    if (url.pathname === '/api/session/register') {
      const participantName = String(body.participantName || 'Participant').trim().slice(0, 60) || 'Participant';
      const normalizedName = normalizeName(participantName);
      const participantId = String(body.participantId || participantName.toLowerCase().replace(/[^a-z0-9]+/g, '-') + '-' + id().slice(0,8));

      const existing = [...sessions.values()].find(session => session.participantId === participantId && session.state !== 'ended');
      if (existing && isLive(existing)) return send(res, 409, { code: 'MULTIPLE_TAB', message: 'Another quiz tab is already active.', session: publicSession(existing) });
      const sameName = [...sessions.values()].find(session => session.normalizedName === normalizedName && session.state !== 'ended' && isLive(session));
      if (sameName) return send(res, 409, { code: 'PARTICIPANT_ALREADY_LOGGED_IN', message: `${sameName.participantName} is already logged in from another quiz session.` });
    ...
    if (url.pathname === '/api/session/heartbeat') {
      const session = findSession(body);
      if (!validIdentity(session, body)) return send(res, 403, { error: 'Unauthorized or expired quiz tab.' });
      session.lastHeartbeat = now(); return send(res, 200, { connected: true });
    }

    if (url.pathname === '/api/session/state') {
      const session = findSession(body);
      if (!validIdentity(session, body)) return send(res, 403, { error: 'Unauthorized or expired quiz session.' });
    ...
    if (url.pathname === '/api/organizer/start') {
      ...
      for (const session of sessions.values()) {
        if (competition.selectedParticipantIds.includes(session.participantName)) {
          session.state = 'quiz';
        }
      }
    ...
    if (url.pathname === '/api/organizer/roster') {
      const students = Array.isArray(body.students) ? body.students.map(name => String(name).trim()).filter(Boolean) : [];
      roster.length = 0;
      roster.push(...students.map(name => ({ id: name.toLowerCase().replace(/[^a-z0-9]+/g, '-'), name })));
    ...
    if (url.pathname === '/api/organizer/questions') {
      ...
      if (!item || !item.prompt || !Array.isArray(item.options) || item.options.length !== 4 || !Number.isInteger(item.correct)) {
  ...
  if (!filePath.startsWith(path.join(__dirname, 'public'))) return send(res, 404, { error: 'Not found' });
...
http.createServer((req, res) => route(req, res).catch(error => send(res, 500, { error: error.message }))).listen(PORT, () => console.log(`CODE BATTLE 2026 listening on http://localhost:${PORT}`));
